...
public class AmazonItem {
    ...
    public synchronized void addPrice(Price<Integer> price) throws IOException, URISyntaxException {
        if (prices.size() > 0) {
            ...
            double lastPrice = last.item() + last.shipping();
            double nextPrice = price.item() + price.shipping();
            if (lastPrice > nextPrice) {
                ...
                if(percentage >= 10){
                    ...
                    TelegramMessageHandler.getInstance().sendMessage(name, url, image, price.item(), price.shipping(), status, seller);
                }
